...
@update_strategy("sim_vit_update_strategy")
def sim_vit_update_strategy(model, train_data, optimizer, use_cuda=True, amp=False, **kwargs) -> t.Dict:
    inputs, targets = train_data
    if use_cuda:
        inputs, targets = inputs.cuda(), targets.cuda()
    inputs, targets = Variable(inputs), Variable(targets)
    losses, out = model(inputs, targets)
    total_loss = compute_loss_value(losses)
    total_loss = total_loss.mean()
    if amp:
